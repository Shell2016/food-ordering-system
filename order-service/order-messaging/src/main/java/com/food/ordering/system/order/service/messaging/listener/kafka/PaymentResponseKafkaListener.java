    ...
    public void receive(@Payload List<PaymentResponseAvroModel> messages,
        ...
        messages.forEach(avroModel -> {
            if (PaymentStatus.COMPLETED == avroModel.getPaymentStatus()) {
                ...
                listener.paymentCompleted(mapper.paymentResponseAvroModelToPaymentResponse(avroModel));
            } else if (PaymentStatus.CANCELLED == avroModel.getPaymentStatus() ||
                    PaymentStatus.FAILED == avroModel.getPaymentStatus()) {
                log.info("Processing cancelling of unsuccessful payment for order id: {}",
                        avroModel.getOrderId());
                listener.paymentCancelled(mapper.paymentResponseAvroModelToPaymentResponse(avroModel));
            }
        });
    }
}
